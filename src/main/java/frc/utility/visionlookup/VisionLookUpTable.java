package frc.utility.visionlookup;
	...
	public ShooterPreset getShooterPreset(double distanceFromTarget){
		...
		for (int i = 1; i < shooterConfig.getShooterConfigs().size(); i++){
			...
			if(dist == distanceFromTarget){
				...
				double percentIn = (distanceFromTarget - shooterConfig.getShooterConfigs().get(i-1).getDistance()) / ( shooterConfig.getShooterConfigs().get(i).getDistance() - shooterConfig.getShooterConfigs().get(i-1).getDistance());
				...
				return interpolateShooterPreset(shooterConfig.getShooterConfigs().get(i-1), shooterConfig.getShooterConfigs().get(i), percentIn);
			}
