...
public class TenBall extends TemplateAuto implements Runnable  {
    ...
    public void run() {
        ...
        p1.addPoint(new Translation2D(243 - 35.0/2+6, -131), 50);
        ...
        p2.addPoint(new Translation2D(125.5, 67), 50);

        p2.addPoint(new Translation2D(120.5, 67), 50);
        ...
        p3.addPoint(new Translation2D(180, 131), 50);
        p3.addPoint(new Translation2D(302, 131), 50);
        drive.setAutoPath(p3, true);
        while(!drive.isFinished()) if(isDead()) return;

        Path p4 = new Path(here());

        p4.addPoint(new Translation2D(120+112, 81),50);
        ...
        p5.addPoint(new Translation2D(120+112+3, 81-14), 50);
        drive.setAutoPath(p5, true);
        while(!drive.isFinished()) if(isDead()) return;

        Path p6 = new Path(here());
        p6.addPoint(new Translation2D(120+112, 81),50);
        p6.addPoint(new Translation2D(120.5+30, 67), 50);
        shooter.setSpeed(5300);
